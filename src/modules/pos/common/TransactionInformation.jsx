...
export default function TransactionInformation({ form, transactionModeData }) {
	...
	const payments = form.values.payments ?? [];
	const isThisTableSplitPaymentActive = payments.length > 1;
	...
	return (
		<>
			<Grid columns={13} gutter={4} justify="center" align="center" pb={4} bg="gray.1">
					...
					<Grid bg="gray.1" px={4}>
						...
						<Grid.Col span={6}>
							<Group justify="space-between">
								...
								<Text fz="sm" fw={800} c="black">
									{calculateVATAmount(10, configData?.inventory_config?.config_vat)}
								</Text>
							</Group>
							<Group justify="space-between">
								<Text fz={"sm"} fw={500} c={"black"}>
			...
			<Grid columns={24} gutter={2} align="center" justify="center" mb={4} bdrs={4}>
				<Grid.Col span={21}>
					<Box mr={4}>
						<Tooltip
							label={t("TransactionMode")}
							opened={!!form.errors.payments}
							px={16}
							py={2}
							bg="orange.8"
							c="white"
								...
								{transactionModeData?.map((mode) => (
									<Carousel.Slide key={mode.id}>
										<Box
											...
											pos="relative"
											className={
												isThisTableSplitPaymentActive ? "cursor-not-allowed" : "cursor-pointer"
											}
										>
											<Flex
												bg={
