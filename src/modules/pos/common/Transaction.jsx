...
export default function Transaction({ form, tableId = null }) {
	...
	const isSplitPaymentActive = form.values.payments.length > 1;
	...
	const handleSave = async ({ withPos = false, status = "completed" }) => {
		...
		if (hasZeroQuantity) {
			showNotification(t("QuantityCantBeZeroCheckCart"), "red", "", "", true, 2000, true);
			return;
		}
		...
		if (!form.values.payments || form.values.payments.length === 0) {
			showNotification(t("ChooseTransactionMode"), "red", "", "", true, 1000, true);
			return;
		}
	...
	const handleOfflineSave = async (fullAmount, status = "completed") => {
		...
		const customerInfo =
			customerObject ||
			customersDropdownData.find((d) => d.id?.toString() == form.values.customer_id);
		...
		<Stack bg="gray.0" align="stretch" justify="center" mt={6} gap={4} pl={4} pr={2} mb={0}>
			...
			<Group gap={6} mb={4} preventGrowOverflow={false} grow align="center" wrap="nowrap">
				<SelectForm
					...
					name="sales_by_id"
					form={form}
					dropdownValue={coreUsers.map((user) => ({
						label: user.name,
					...
					<Grid.Col span={6}>
						...
						>
							<Button
								...
								px="2xs"
								leftSection={
									customerObject?.name ? <></> : <IconUserPlus height={14} width={14} stroke={2} />
								}
			...
			<Grid columns={17} gutter={{ base: 2 }}>
				<Grid.Col span={2}>
					<Tooltip label="Reset (alt+r)" withArrow position="top">
						<ActionIcon
							size="xl"
							h="100%"
							w="100%"
