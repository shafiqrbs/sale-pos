...
export default function Form({
    ...
    const { data: customerGroupData } = useGetDropdownDataQuery({ 'dropdown-type': 'customer-group' });
    ...
    const handleConfirmSubmit = async (values) => {
        if (mode === 'create') {
            ...
            if (response.data) {
                ...
                showNotification(t("CreateFailed"), "red", "", <IconX style={{ width: rem(18), height: rem(18) }} />);
            }
        } else {
            ...
            if (response.data) {
                ...
                showNotification(t("UpdateFailed"), "red", "", <IconX style={{ width: rem(18), height: rem(18) }} />);
            }
        }
    };
    ...
    return (
        <Box>
            <Box component="form" onSubmit={form.onSubmit(handleSubmit)}>
                <Box bg="white">
                    <LoadingOverlay visible={isCreateCustomerLoading || isUpdateCustomerLoading} zIndex={1000} overlayProps={{ radius: "sm", blur: 2 }} loaderProps={{ color: 'red.6' }} />
                            ...
                            <Box>
                                    ...
                                    <Grid.Col span={12}>
                                        <Box mt="8">
                                            <SelectForm
                                                ...
                                                form={form}
                                                dropdownValue={customerGroupData?.data?.map(item => ({ value: String(item.id), label: item.name }))}
                                                mt={8}
                                ...
                                <Box mt={"xs"}>
                                    <SelectForm
                                        ...
                                        id="location_id"
                                        form={form}
                                        dropdownValue={locationData?.data?.map(item => ({ value: String(item.id), label: item.name }))}
                                        mt={8}
                                        searchable={true}
                                ...
                                <Box mt={"xs"}>
                                    <SelectForm
                                        ...
                                        name="marketing_id"
                                        form={form}
                                        dropdownValue={marketingExeData?.data?.map(item => ({ value: String(item.id), label: item.name }))}
                                        id="marketing_id"
                                        searchable={true}
